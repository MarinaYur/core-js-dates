...
function isDateInPeriod(date, period) {
  const msStart = +new Date(period.start);
  const msEnd = +new Date(period.end);
  const msDate = +new Date(date);
  return msDate >= msStart && msDate <= msEnd;
}
...
function formatDate(date) {
  ...
  const fullYear = date1.getUTCFullYear();
  const month = date1.getUTCMonth() + 1;
  const day = date1.getUTCDate();
  const time = date1.toLocaleTimeString('en-US', { timeZone: 'UTC' });
  return `${month}/${day}/${fullYear}, ${time}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const daysFromYearBeginToDate =
    (new Date(date) - yearBegin) / (1000 * 60 * 60 * 24);
  return Math.round((daysFromYearBeginToDate + 1) / 7);
}
...
function getQuarter(date) {
  ...
  if ([1, 2, 3].includes(month + 1)) {
    quarter = 1;
  } else if ([4, 5, 6].includes(month + 1)) {
    quarter = 2;
  } else if ([7, 8, 9].includes(month + 1)) {
    quarter = 3;
  } else {
    quarter = 4;
  }
 ...
 */
function getWorkSchedule(/* uperiod, countWorkDays, countOffDays */) {
